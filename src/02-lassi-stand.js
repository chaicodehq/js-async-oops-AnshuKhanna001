...
LassiStand.prototype.addFlavor = function (flavor, price) {
  if (price <= 0) return -1;
  const flavorExists = this.menu.some((item) => item.flavor === flavor);
  if (flavorExists) return -1;
...
LassiStand.prototype.completeOrder = function (orderId) {
  const order = this.orders.find((id) => orderId === id.id);
...
LassiStand.prototype.getRevenue = function () {
  const getCompletedOrders = this.orders.filter(
    (order) => order.status === "completed",
  );
  return getCompletedOrders.reduce((sum, order) => sum + order.total, 0);
};

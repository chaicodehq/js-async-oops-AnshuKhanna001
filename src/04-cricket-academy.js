...
export class Batsman extends Player {
  ...
  getStrikeRate() {
    ...
    const totalSR = this.innings.reduce((sum, inn) => {
      return sum + inn.strikeRate;
    }, 0);
  ...
  getProfile() {
    // Your code here
    const totalRuns = this.innings.reduce((sum, inn) => {
      return sum + inn.runs;
    }, 0);
...
export class Bowler extends Player {
  ...
  getEconomy() {
    // Your code here
    const averageEconomy = this.spells.reduce((sum, s) => {
      return sum + s.economy;
    }, 0);
    return averageEconomy / this.spells.length;
  }

  getProfile() {
    // Your code here
    const totalWickets = this.spells.reduce((sum, inn) => {
      return sum + inn.wickets;
    }, 0);

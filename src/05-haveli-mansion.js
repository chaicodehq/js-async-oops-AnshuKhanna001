...
export class HaveliSecurity {
  ...
  addResident(name, role, passcode) {
    // Your code here
    const allowedRole = ["malik", "naukar", "mehmaan"];
    if (passcode !== this.#passcode)
      return { success: false, message: "Galat passcode!" };
    if (!allowedRole.includes(role))
      return { success: false, message: "Invalid role!" };
    const nameExist = this.#residents.find((r) => r.name === name);
    if (nameExist) return { success: false, message: "Already a resident!" };
  ...
  verifyAccess(name) {
    // Your code here
    const nameExists = this.#residents.find((n) => n.name === name);
    if (nameExists) {
      this.#accessLog.push({
        name,
